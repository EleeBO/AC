 ...
 *   cc -O2 -c ac.c -o ac.o
 *   cc ac.o notorch.o loragrad.o -O2 -lm -lpthread -o ac
 *
 * Or simply:  make
 *
 * Status:
 ...
 *   [TODO] persistence (ac.state, .spores, .scars, .dark)
 *
...
#define ac_BLOCK_SIZE     64       /* context length                            */
#define ac_N_LAYER         2       /* transformer depth                         */
#define ac_N_EMBD         64       /* embedding / hidden width                  */
#define ac_N_HEAD          4       /* attention heads                           */
#define ac_HEAD_DIM       (ac_N_EMBD / ac_N_HEAD)
#define ac_MLP_DIM        (4 * ac_N_EMBD)
#define ac_VOCAB_MAX     128       /* char vocab — printable ASCII upper bound  */

#define ac_LG_EXPERTS      8       /* parliament size                           */

/* Training */
#define ac_TRAIN_STEPS   2000      /* smoke: overfit quickly                    */
#define ac_LR            3e-4f
#define ac_LOG_EVERY       50
#define ac_GEN_LEN        200      /* tokens to generate after training         */
...
typedef struct {
    int   vocab_size;
    char  id2char[ac_VOCAB_MAX];
    int   char2id[256];
} ac_tokenizer;

static void ac_tok_init(ac_tokenizer* tk) {
    memset(tk, 0, sizeof(*tk));
    for (int i = 0; i < 256; ++i) tk->char2id[i] = -1;
}

static void ac_tok_fit(ac_tokenizer* tk, const char* corpus, int len) {
    int seen[256] = {0};
    for (int i = 0; i < len; ++i) seen[(unsigned char)corpus[i]] = 1;
    int next_id = 0;
    for (int c = 0; c < 256; ++c) {
        if (seen[c] && next_id < ac_VOCAB_MAX - 1) {
...
static int ac_tok_encode_char(const ac_tokenizer* tk, unsigned char c) {
    int id = tk->char2id[c];
...
static int ac_origin_load(ac_origin* org, const char* path) {
...
static void ac_origin_free(ac_origin* org) {
    if (!org) return;
...
static const char* ac_BOUNDARY_SEED[] = {
...
static int ac_immune_init(lg_field_t* f, const ac_origin* org, uint64_t seed) {
    int rc = lg_field_init(f, ac_LG_EXPERTS, seed);
    if (rc != 0) return rc;

    lg_field_set_origin_from_sketches(f, org->sketches, org->n_lines);

    int n_b = 0;
    while (ac_BOUNDARY_SEED[n_b]) n_b++;
    float* b_sk = (float*)calloc((size_t)n_b * LG_SIG_DIM, sizeof(float));
    for (int i = 0; i < n_b; ++i) {
        const char* s = ac_BOUNDARY_SEED[i];
...
static void ac_model_init(ac_model* m, int vocab_size) {
    memset(m, 0, sizeof(*m));
    m->vocab_size = vocab_size;
    m->n_layer    = ac_N_LAYER;
    m->n_embd     = ac_N_EMBD;
    m->n_head     = ac_N_HEAD;
    m->head_dim   = ac_HEAD_DIM;
    m->block_size = ac_BLOCK_SIZE;
    int E = ac_N_EMBD;
    ...
    for (int l = 0; l < m->n_layer; ++l) {
        ...
        m->L[l].w_up   = nt_tensor_new2d(ac_MLP_DIM, E);
        nt_tensor_xavier(m->L[l].w_up, E, ac_MLP_DIM);
        m->L[l].w_down = nt_tensor_new2d(E, ac_MLP_DIM);
        nt_tensor_xavier(m->L[l].w_down, ac_MLP_DIM, E);
...
static int ac_param_count(const ac_model* m) {
...
static void ac_model_free(ac_model* m) {
    if (!m) return;
...
static int ac_forward(ac_model* m, int* tokens, int* targets) {
    int T = m->block_size;
    int E = m->n_embd;
    int V = m->vocab_size;
...
static void ac_train(ac_model* m, lg_field_t* field, ac_tokenizer* tk,
                      int* encoded, int n_chars, int steps)
{
    ...
    printf("  steps=%d  lr=%.1e  ctx=%d  vocab=%d  params=%d\n",
           steps, ac_LR, ac_BLOCK_SIZE, tk->vocab_size, ac_param_count(m));
    printf("  corpus: %d chars (%.1f KB)\n", n_chars, n_chars / 1024.0);
    printf("══════════════════════════════════════════════════════════════════\n\n");

    int T = ac_BLOCK_SIZE;
    nt_schedule sched = nt_schedule_cosine(ac_LR, steps / 10, steps, ac_LR * 0.1f);
    nt_nan_guard guard = nt_nan_guard_new();
    ac_train_stats stats = {0};
    ...
    for (int step = 0; step < steps; ++step) {
        ...
        int tokens[ac_BLOCK_SIZE], targets[ac_BLOCK_SIZE];
        for (int i = 0; i < T; ++i) {
        ...
        /* ── Forward ───────────────────────────────────────────────────── */
        nt_tape_start();
        int loss_idx = ac_forward(m, tokens, targets);
        ...
        {
            char window_text[ac_BLOCK_SIZE + 1];
        ...
        /* ── Logging ───────────────────────────────────────────────────── */
        if ((step + 1) % ac_LOG_EVERY == 0 || step == 0) {
    ...
    double elapsed = (ac_now_ms() - t0) / 1000.0;
    printf("\n── training complete ──\n");
...
static void ac_generate(ac_model* m, ac_tokenizer* tk, const char* prompt,
                         int max_tokens, float temp)
{
    int T = m->block_size;
    int V = m->vocab_size;
    int ctx[ac_BLOCK_SIZE];
    int gen_len = 0;

    /* Encode prompt */
    for (int i = 0; prompt[i] && gen_len < T / 2; ++i)
        ctx[gen_len++] = ac_tok_encode_char(tk, (unsigned char)prompt[i]);
...
static const char* ac_TEST_ALIGNED[] = {
...
static const char* ac_TEST_VIOLATING[] = {
...
static void ac_smoke_immune(lg_field_t* field) {
    printf("\n── immune smoke ──\n");
    ...
    for (int i = 0; ac_TEST_ALIGNED[i]; ++i) {
        const char* s = ac_TEST_ALIGNED[i];
    ...
    for (int i = 0; ac_TEST_VIOLATING[i]; ++i) {
        const char* s = ac_TEST_VIOLATING[i];
        float alpha, sig[LG_SIG_DIM];
        lg_signature_from_text(s, (int)strlen(s), sig);
        lg_verdict_t v = lg_field_vote(field, sig, &alpha);
        lg_field_record(field, v, sig);
...
int main(int argc, char** argv) {
    ...
    int         train_steps = (argc > 2) ? atoi(argv[2]) : ac_TRAIN_STEPS;
    ...
    /* ── L-1: load origin ────────────────────────────────────────────────── */
    ac_origin org = {0};
    if (ac_origin_load(&org, origin_path) != 0) {
    ...
    lg_field_t field;
    if (ac_immune_init(&field, &org, seed) != 0) {
    ...
    ac_smoke_immune(&field);

    /* ── L1: tokenizer + model ───────────────────────────────────────────── */
    ac_tokenizer tok;
    ac_tok_init(&tok);
    ac_tok_fit(&tok, org.buf, org.len);
    ...
    int n_chars = 0;
    for (int i = 0; i < org.len; ++i) {
        int id = ac_tok_encode_char(&tok, (unsigned char)org.buf[i]);
        encoded[n_chars++] = id;
    }
    printf("[L1]  corpus encoded: %d tokens\n", n_chars);

    if (n_chars < ac_BLOCK_SIZE + 2) {
        fprintf(stderr, "fatal: corpus too small (%d < %d)\n", n_chars, ac_BLOCK_SIZE + 2);
        free(encoded);
        ac_origin_free(&org);
        return 3;
    }
    ...
    ac_model model;
    ac_model_init(&model, tok.vocab_size);
    printf("[L1]  model: %d layers, %d embd, %d heads, %d params (%.2fK)\n",
           model.n_layer, model.n_embd, model.n_head,
           ac_param_count(&model), ac_param_count(&model) / 1000.0);

    /* ── Train ───────────────────────────────────────────────────────────── */
    lg_field_reset_counters(&field);
    ac_train(&model, &field, &tok, encoded, n_chars, train_steps);
    ...
    for (int p = 0; p < 3; ++p) {
        ac_generate(&model, &tok, prompts[p], ac_GEN_LEN, 0.8f);
        printf("\n");
    }
    ...
    /* ── Cleanup ─────────────────────────────────────────────────────────── */
    printf("\n──────────────────────────────────────────────────────────────────\n");
    printf("ac phase 1 complete. L0 + L1 verified.\n");
